            ...
            stage('last test') {
                withCredentials([usernamePassword(credentialsId: 'application', passwordVariable: 'APPLICATION_PASSWORD', usernameVariable: 'APPLICATION_USER_NAME')]) {
                    container('maven') {
                        ...
                        sh "mvn -s settings.xml clean jmeter:jmeter -Dlt.domain=${HOST} -Dlt.port=${PORT} -Dlt.keycloak_token=${data.access_token} -Dlt.path=/sink/resources/users -Dlt.threads=20"
