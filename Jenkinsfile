    ...
    node {
        ...
        stage('deploy to prod') {
            ...
            withCredentials([usernamePassword(credentialsId: 'github-api-token', passwordVariable: 'GITHUB_TOKEN', usernameVariable: 'GIT_USERNAME')]) {
                sh "curl --data '{\"tag_name\": \"${env.VERSION}\",\"target_commitish\": \"master\",\"name\": \"${env.VERSION}\",\"body\": \"Release of version ${env.VERSION}\",\"draft\": false,\"prerelease\": false}' https://api.github.com/repos/khinkali/sink/releases?access_token=${GITHUB_TOKEN}"
            }
